...
function select(word,selected_words,lang,list_words,possible_sent,turn,position,new_position)
{
	...
	$('#words_sentence').load('CLexp2.php?lang='+lang+'&words_selected='+selected_words+'&words='+list_words+'&turn='+turn+'&possible_sent='+possible_sent+'&position='+position);
}
...
function clears(list_words,lang,turn,possible_sent,position)
{
	...
	for(temp in position)
		if(position[temp].length==0)
			delete(position[temp]);
	$('#words_sentence').load('CLexp2.php?lang='+lang+'&words_selected=%&words='+list_words+'&turn=0'+'&possible_sent='+possible_sent+'&position=%');
}
...
function getAnswer(possible_sent)
{
	...
	$('#answer').load('CLexp2_ans.php?possible_sent='+possible_sent);
}
...
echo "<div style='color:#0000AA;font-size:16px'><br/><br/><b>Form a  sentence (Declarative or Interrogative or any other type) from all of the given words</b></div><div style='color:#0000FF;font-size:14px' <b><i>(select the buttons in proper order, the submit option only pops up when all the buttons are selected.) </i></b><br/><br/></div>";
